...
<div class="ticket">
            <p class="centered">
                ...
                <br><?php echo $store['name']; ?>
                <br><?php echo $store['address']; ?></p>
            <p class="centered">
                قام (<?php
                $user1 = $this->stores_model->getByData('users',' WHERE (id = '.$movestock['u_id'].')');
                echo $user1[0]['id'].' - '.$user1[0]['name'];
                ?>) بإنشاء طلب النقل و
                قام (<?php
                $user2 = $this->stores_model->getByData('users',' WHERE (id = '.$movestock['with_u_id'].')');
                echo $user2[0]['id'].' - '.$user2[0]['name'];
                ?>) بالنقل
                وقام (<?php
                $user3 = $this->stores_model->getByData('users',' WHERE (id = '.$movestock['with_u_id'].')');
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="description centered"><?php
                            $product = $this->stores_model->getByData('products',' WHERE (created_at = \''.$movestock['p_created_at'].'\')');
...
<a href="<?php echo base_url('AdminControllerStores/products/'.$store['id']); ?>" class="btn-print hidden-print">رجوع للمحل</a>
...
<script>
    ...
    if (event == 27) { // ESC Key
        window.location.href = '<?php echo base_url('AdminControllerStores/products/'.$store['id']); ?>'; // Navigate to URL
    }
}
</script>
</body>
