...
<div class="ticket">
            <p class="centered">
                ...
                <br><?php echo $factory['name']; ?>
                <br><?php echo $factory['address']; ?></p>
            <p class="centered">
                قام (<?php
                $user1 = $this->factories_model->getByData('users',' WHERE (id = '.$movestock['u_id'].')');
                echo $user1[0]['id'].' - '.$user1[0]['name'];
                ?>) بإنشاء طلب النقل و
                قام (<?php
                $user2 = $this->factories_model->getByData('users',' WHERE (id = '.$movestock['with_u_id'].')');
                echo $user2[0]['id'].' - '.$user2[0]['name'];
                ?>) بالنقل
                وقام (<?php
                $user3 = $this->factories_model->getByData('users',' WHERE (id = '.$movestock['with_u_id'].')');
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="description centered"><?php
                            $product = $this->factories_model->getByData('products',' WHERE (id = '.$movestock['p_id'].')');
...
<a href="<?php echo base_url('AdminControllerFactories/products/'.$factory['id']); ?>" class="btn-print hidden-print">رجوع للمصنع</a>
...
<script>
    ...
    if (event == 27) { // ESC Key
        window.location.href = '<?php echo base_url('AdminControllerFactories/products/'.$factory['id']); ?>'; // Navigate to URL
    }
}
</script>
</body>
